...
class ArUcoDetectionSystem:
    """
    ArUco detection system with YOUR actual marker measurements
    """
    
    def __init__(self, camera_id=1, calibration_file="F:/swarm_robots/codes/computer_vision/camera_calibration/motorola_calibration_20260702_215248.npz"):
        """
        Initialize detection system with your marker sizes
        """
        ...
        # ============================================================
        # YOUR ACTUAL MEASUREMENTS - UPDATE THESE!
        # ============================================================
        self.marker_sizes_mm = {
            ...
            20: 33.5, 21: 33.5,  # Using average: (30+37)/2 = 33.5
            
            # Robot markers (IDs 100-102) - 27x37mm (rectangular)
            100: 32, 101: 32, 102: 32,  # Using average: (27+37)/2 = 32
        }
        ...
        # Workspace settings
        self.workspace_width_mm = 2000
        self.workspace_height_mm = 1500
        ...
        print("="*60)
        print("📏 YOUR MARKER MEASUREMENTS:")
        ...
        # Print the actual marker sizes being used
        print("\n📊 USING THESE SIZES FOR DETECTION:")
        for marker_id, size in self.marker_sizes_mm.items():
            dims = self.marker_actual_dimensions.get(marker_id, (size, size))
            print(f"   ID {marker_id}: {dims[0]}x{dims[1]}mm -> Using {size:.1f}mm avg")
    ...
    def detect_markers(self, frame):
        ...
        if ids is not None and len(ids) > 0:
            for i in range(len(ids)):
                ...
                # Calculate size in pixels (average of width and height)
    ...
    def get_all_positions(self):
        ...
        # Update robot positions (IDs 100-102)
        ...
        # Update task locations (IDs 10, 11, 20, 21)
    ...
    def get_smoothed_position(self, robot_id):
        """Get smoothed position for robot"""
    ...
    def draw_markers(self, frame, positions):
        ...
        for marker_id, data in positions.items():
            ...
            dims = data['actual_dimensions']
            dims_text = f"{dims[0]}x{dims[1]}mm"
            cv2.putText(frame, dims_text, (center[0]-25, center[1]-5),
    ...
    def draw_info_overlay(self, frame, positions):
    ...
    def run_detection(self):
        ...
        print("="*60)
        print("📏 Using YOUR actual marker measurements")
        print("Controls:")
        print("  q - Quit")
        print("  s - Save layout")
            ...
            if not paused:
                ...
                if not self.is_workspace_setup:
                    ...
                    if success:
                        ...
                        cv2.putText(frame, "Place boundary markers (IDs 0-3)", (10, 30),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                
                self.detect_markers(frame)
                positions = self.get_all_positions()
                self.draw_markers(frame, positions)
                self.draw_info_overlay(frame, positions)
                ...
                if self.is_workspace_setup:
                    ...
                    display_warped = cv2.resize(warped, (800, 600))
                    cv2.imshow('Top-Down View', display_warped)
            
            cv2.imshow('ArUco Detection System', frame)
    ...
    def save_layout(self):
        ...
        with open(txt_filename, 'w') as f:
            ...
            f.write("MARKER MEASUREMENTS:\n")
            for id, dims in self.marker_actual_dimensions.items():
                f.write(f"  ID {id}: {dims[0]}x{dims[1]}mm\n")
...
if __name__ == "__main__":
    print("🚀 ArUco Detection with YOUR Marker Measurements")
    print("="*60)
    ...
    if calibration_file:
        ...
        print("   The system will run without calibration")
